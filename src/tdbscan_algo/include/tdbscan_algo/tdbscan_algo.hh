...
TDBScan_Algo<tBlib>::Process (const tBlibContainer& inhits) {
  ...
  AbsHitSet hs_to; // time-order
  for (const auto& hit : inhits)
    hs_to.insert(h);

  for (const auto& hit : hs_to)
...
template <class tBlib>
void TDBScan_Algo<tBlib>::AddHit (const tBlib& h) {
  ...
  while (ac_iter != active_clusters_.end()) {
    ...
    ::AvanceInTime(*ac_iter, now); // check
  ...
  while (ac_iter != active_clusters_.end()) {
    ...
    if (ac_iter->GetCount() == n_overlap) {
      ...
      active_clusers_.insert(active_clusters_.end(), ::ConstructSubCluster(*ac_iter, h));
    }
  }
...
bool TDBScan_Algo::AddHitToCluster (
  CausalCluster& c,
  const AbsHit& h)
{
