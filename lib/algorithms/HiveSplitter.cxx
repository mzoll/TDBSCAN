...
bool hivesplitter::detail::CausallyOverlaps(
  ...
  while (iter1 != end1 && iter2 != end2) {
    if (*iter1 < *iter2)
      ...
      //*iter1==*iter2 ; Hits are identical in time and DOM
...
hivesplitter::detail::CausalCluster::CausalCluster(
  const HiveSplitter_ParameterSet *p) : params(p),
...
void hivesplitter::detail::CausalCluster::insertActiveHit(const AbsHit &h) {
  ...
  if (active_doms.size() >= params->multiplicity) {
    // TODO watch out; this is outside info
    established = true;
  }
}
...
void hivesplitter::detail::CausalCluster::advanceInTime(
  ...
  while (!active_hits.empty()) {
    ...
    if (time > ah_iter->GetTime() + params->multiplicityTimeWindow) {
      // TODO watch out; this is outside info
      //the hit is no longer active, thus
      //decrement the number of hits on the DOM where h occurred
      if ((--active_doms[ah_iter->GetDOMIndex()]) <= 0)
        //NOTE TODO do we need to bother with this after the cluster is established, and this is probably not checked anymore?
...
HiveSplitter::HiveSplitter(const hivesplitter::HiveSplitter_ParameterSet &params) : params_(params) {
...
void HiveSplitter::AddHit(const AbsHit &h) {
  ...
  while (nc_iter != newClusters_.end()) {
    ...
    while (c_iter != clusters_.end()) {
      ...
      if (h == c_iter->getLatestActiveHit()) {
        // FIXME this might break because of the sorting if two hits have the exact same hit-time but DOM-index forces a strict ordering (EDGECASE)
  ...
  if (!c.getFirstHitTimes().count(h.GetDOMIndex())) {
    ...
    for (; it != end; ++it) {
      if (CausallyConnected(*it, h, params_.connectorBlock)) {
        ...
        //exit condition
        if (connectedDOMs.size() >= params_.multiplicity - 1) {
    ...
    for (; it != end; ++it) {
      if (it->GetDOMIndex() == h.GetDOMIndex()) {
        ...
        if (dt <= params_.acceptTimeWindow) {
          // it and h connected
          connectedHits.insert(connectedHits.begin(), *it);
          continue;
        }

        if (dt > params_.rejectTimeWindow) {
          // it rejects h, so not connected
          allConnected = false;
          continue;
        }
...
void HiveSplitter::AddSubEvent(AbsHitSet newSet) {
  ...
  while (pse_iter != partialSubEvents_.end()) {
    ...
    const bool sufficent_overlap = CausallyOverlaps(newSet, *pse_iter, params_.mergeOverlap,
                                                    params_.multiplicityTimeWindow);
